...
- Policy Model: Qwen/Qwen3-4B (Now available on Hugging Face)
- Reward Model: Qwen/Qwen3-4B trained as reward model using RewardTrainer
...
def setup_models_and_tokenizer(model_config: ModelConfig, training_args: RLOOConfig, reward_model_path: Optional[str] = None):
    ...
    # Using Qwen3-4B which is now available on Hugging Face
    policy_model_name = "Qwen/Qwen3-4B"
    
    # Use trained Qwen3-4B reward model if path provided, otherwise use base model
    if reward_model_path:
        reward_model_name = reward_model_path
        print(f"Using trained reward model from: {reward_model_name}")
    else:
        reward_model_name = "experiment/models/qwen3_4b_reward_model"  # Default path for our trained model
    ...
    reward_model = AutoModelForSequenceClassification.from_pretrained(
        reward_model_name,
        num_labels=1,  # Explicitly set to 1 to match trained reward model
        trust_remote_code=model_config.trust_remote_code,
...
def main():
    ...
    # Get reward model path from script_args (should be available in ScriptArguments)
    ...
    print(f"Reward Model: {reward_model_path} (Qwen/Qwen3-4B trained)")
