#!/usr/bin/env python3
"""
Fixed Multi-GPU RLOO Training Script with English and Chinese Data
Uses 22,156 English samples and 224 Chinese samples from HelpSteer3
"""
...
def filter_language_samples(dataset, language, max_samples=None):
    """Filter dataset for specific language samples and optionally limit the count"""
    print(f"Filtering for {language} language samples using 'language' column...")
    filtered_dataset = dataset.filter(lambda x: x.get('language', '').lower() == language.lower())
    
    if max_samples and len(filtered_dataset) > max_samples:
        print(f"Randomly selecting {max_samples} samples from {len(filtered_dataset)} {language} samples")
    ...
    print(f"Selected {len(filtered_dataset)} {language} samples")
    return filtered_dataset

def prepare_datasets(args, tokenizer, rank=0):
    """Prepare training and evaluation datasets with English and Chinese data"""
    ...
    try:
        ...
        # Filter for English samples (22,156 samples)
        english_dataset = filter_language_samples(dataset, "english", max_samples=22156)
        timeout_handler.check_timeout("after English filtering")

        # Filter for Chinese samples (224 samples)
        chinese_dataset = filter_language_samples(dataset, "chinese", max_samples=224)
        timeout_handler.check_timeout("after Chinese filtering")

        # Combine English and Chinese datasets
        print("Combining English and Chinese datasets...")
        combined_dataset = concatenate_datasets([english_dataset, chinese_dataset])
        print(f"Combined dataset size: {len(combined_dataset)} samples")
        timeout_handler.check_timeout("after dataset combination")
        ...
        def convert_to_chosen_rejected(example):
            ...
            if preference == 0:
                return {'chosen': None, 'rejected': None, 'language': language}
            ...
            if preference > 0:
                return {'chosen': full_text1, 'rejected': full_text2, 'language': language}
            else:
                return {'chosen': full_text2, 'rejected': full_text1, 'language': language}

        column_names = combined_dataset.column_names
        reward_dataset = combined_dataset.map(
        ...
        # Count by language
        english_count = len(reward_dataset.filter(lambda x: x['language'].lower() == 'english'))
        chinese_count = len(reward_dataset.filter(lambda x: x['language'].lower() == 'chinese'))
        
        print(f"Converted dataset composition:")
        print(f"  English samples: {english_count}")
        print(f"  Chinese samples: {chinese_count}")
        print(f"  Total samples: {len(reward_dataset)}")

        if len(reward_dataset) < 100:
            raise ValueError(f"Not enough samples: {len(reward_dataset)}")

        # Split: 90% train, 10% eval (maintaining language distribution)
        ...
        # Count train/eval split by language
        train_english = len(train_dataset.filter(lambda x: x['language'].lower() == 'english'))
        train_chinese = len(train_dataset.filter(lambda x: x['language'].lower() == 'chinese'))
        eval_english = len(eval_dataset.filter(lambda x: x['language'].lower() == 'english'))
        eval_chinese = len(eval_dataset.filter(lambda x: x['language'].lower() == 'chinese'))
        
        print(f"Training set composition:")
        print(f"  English: {train_english}, Chinese: {train_chinese}")
        print(f"Evaluation set composition:")
        print(f"  English: {eval_english}, Chinese: {eval_chinese}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="RLOO Training Script with English and Chinese Data")
    ...
    parser.add_argument("--output_dir", type=str, default="experiment/models/qwen3_1.7b_rloo_model_chinese_1", help="Output directory")
    ...
    parser.add_argument("--run_name", type=str, default="rloo_training_chinese", help="Run name")
    ...
    if rank == 0:
        print("🚀 FIXED Multi-GPU RLOO Training with English and Chinese Data")
        ...
        print(f"  Dataset: nvidia/HelpSteer3 (English: 22,156 + Chinese: 224)")
    ...
    try:
        ...
        print("🏃 Starting RLOO training with multilingual data...")
